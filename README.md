...
- **Column-spanning blocks** (with their own colour, and an optional custom room-name label in place of the real room name) cover plenaries and breaks, and are editable in place. The **Add span block** dialog never asks for a date — a new block lands on whichever day is currently selected in the grid, defaulting to a sensible 12:00–13:00 (lunchtime) slot you can adjust. A span block can also be marked a **container** — a poster session or keynote panel holding several nested presentations that share the container's own time and room, added via a "+" button, exempt from the normal overlap check, and rendered as equal-width columns. Nested presentations move only with their container (they cannot be rescheduled on their own), and a talk can only ever be scheduled once per instance — a second placement is refused. A nested presentation's own block never repeats the room/time — that's already shown once on the container's line, in its own clear space below the child tiles. The "+" button opens a filterable **multi-select** picker (filter by track or submission type, select several presentations, save once); each nested tile shows a **track pill** when applicable, and a container's own **text alignment** (horizontal and vertical) for its nested tiles is configurable in the same form.
...
- A **my timetable** toggle highlights your favourited talks and dims the rest — a container session holding one of your starred presentations stays at full strength, with only its unstarred tiles dimmed. An **Export (.ics)** link downloads your favourites as an iCalendar file.
