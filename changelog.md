...
- Original request: "ICS/calendar export of a user's 'my timetable'." New
  ...
  favourited, scheduled presentations as an iCalendar file via a new plain
  (non-AJAX) `export.php` endpoint, matching core's own
  `calendar/export_execute.php`/`mod_confcheckin`'s `badge.php` download
  pattern. New `classes/local/ics_export.php` builds the `.ics` content using
  Moodle's own Bennu library (`lib/bennu`), reusing
  `\mod_confscheduler\local\grid_data::build()`'s already-resolved slot/room/
  favourite data rather than re-querying it -- see README.md's "Architecture
  notes" for why. No schema change (favourite state already exists via
  `mod_confprogram`); `version.php` bumped to `2026070610` as a plain,
  non-savepoint version bump (no matching `install.xml`/`upgrade.php` entry
  needed). 117/117 PHPUnit passing (was 113, +4 new), phpcs/moodlecheck
  clean, EN/JA lang parity verified (110/110 keys).
- User request (2026-07-06): "Each of the four plugins should have a
  ...
  that plugin." New `confscheduler.notificationsenabled` (default 1,
  enabled) checkbox on the existing notifications.php template screen.
  `notifier::notify_slot()` now returns a bool (true only if a send was
  actually attempted); `api::send_pending_notifications()` only marks a
  slot's `notifiedtime` when it returns true and its own return value now
  reflects actual sends -- a slot with a pending change is never silently
  marked "notified" while notifications are disabled, so a later re-enable
  still delivers it. 113/113 PHPUnit passing (was 112), phpcs/moodlecheck
  clean, EN/JA lang parity verified (109/109 keys).
